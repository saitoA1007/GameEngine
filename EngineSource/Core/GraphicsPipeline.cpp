...
void GraphicsPipeline::Initialize(const std::wstring& vsPath, const std::wstring& psPath, ID3D12Device* device, LogManager* logManager) {
	...
	// Shaderをコンパイルする
	vertexShaderBlob_ = CompileShader(vsPath,
		L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(psPath,
